...
for (food, isYum) in polarizingFoods {
    if isYum {
        print("\(food), yum!")
    } else {
        print("\(food), yuck!")
...
for (area, phoneNumber) in rapperPhoneNumbers {
    if phoneNumber.contains("(415)") {
        rapperPhoneNumbers[area]?.replacingOccurrences(of: "(415)", with: "(510)")
    }
}
//: ## Switch Statements
...
case (.rock, .paper), (.paper, .rock): resultsMessage = "Paper covers Rock."
case (.rock, .scissors), (.scissors, .rock): resultsMessage = "Rock crushes Scissors."
case (.paper, .scissors), (.scissors, .paper): resultsMessage = "Scissors cut Paper."
default: resultsMessage = "It's a tie!"
}
//: ### Exercise 7
